...
@Configuration
public class SpringFoxConfiguracao {
                        ...
                        .map(
                                entry ->
                                        new WebMvcRequestHandler(
                                                contextPath, methodResolver, tweakInfo(entry.getKey()), entry.getValue()))
    ...
    RequestMappingInfo tweakInfo(RequestMappingInfo info) {
        ...
        if (pathPatternsCondition != null) {
            patterns = pathPatternsCondition.getPatternValues().toArray(String[]::new);
        } else {
            return info;
        }
        return info.mutate()
